#!/usr/bin/env python3
"""
Generate all test patterns and create visualizations.
"""
...
def process_pattern(name, cloud, metadata, cell_size=1.0):
    """Process a pattern through the pipeline and save output."""
    print(f"\n{'='*70}")
    ...
    config.output_path = f"pattern_outputs/{name}.tif"

    print(f"  Point cloud: {cloud.count():,} points")
...
def main():
    print("="*70)
    print("GENERATING ALL TEST PATTERNS")
    print("="*70)
    ...
    # -------------------------------------------------------------------------
    # Process all patterns through pipeline
    # -------------------------------------------------------------------------

    print("\n" + "="*70)
    print("PROCESSING PATTERNS THROUGH PIPELINE")
    print("="*70)
    ...
    print("="*70)
    print(f"\nOutput directory: pattern_outputs/")
    print(f"Generated {len(outputs)} patterns:")
    for name, _ in outputs:
        print(f"  - {name}.tif / {name}.png")

    print("\nView the images:")
    print("  ls -lh pattern_outputs/*.png")
    print("  eog pattern_outputs/*.png  # or your image viewer")
